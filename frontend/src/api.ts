...
export function createAdminHeaders(headers?: HeadersInit) {
...
export async function readAdminErrorResponse(res: Response) {
  ...
  return extractAdminErrorMessage(body, res.status)
...
export function adminFetch(path: string, options: RequestInit = {}) {
  ...
  return fetch(BASE + path, {
    ...
    headers: createAdminHeaders(options.headers),
...
export const api = {
  ...
  getCPASyncStatus: () =>
    ...
    request<CPASyncStatusResponse>('/cpa-sync/status'),

  runCPASync: () =>

    request<CPASyncStatusResponse>('/cpa-sync/run', { method: 'POST' }),

  switchCPASync: () =>

    request<CPASyncStatusResponse>('/cpa-sync/switch', { method: 'POST' }),

  testCPASyncCPA: (data: Partial<SystemSettings>) =>

    request<ConnectionTestStatus>('/cpa-sync/test-cpa', { method: 'POST', body: JSON.stringify(data) }),

  testCPASyncMihomo: (data: Partial<SystemSettings>) =>

    request<ConnectionTestStatus>('/cpa-sync/test-mihomo', { method: 'POST', body: JSON.stringify(data) }),

  listCPASyncMihomoGroups: (data: Partial<SystemSettings>) =>

    request<{ groups: MihomoStrategyGroupOption[] }>('/cpa-sync/mihomo-groups', { method: 'POST', body: JSON.stringify(data) }),
