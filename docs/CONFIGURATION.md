...
- `ADMIN_SECRET` 现在是**启动必填**，环境变量和数据库都为空时服务会直接启动失败。
- `.env.example` 与 `.env.sqlite.example` 已同步要求设置 `ADMIN_SECRET`。
- `CODEX_API_KEYS` 与 `CODEX_PROXY_URL` 的旧兼容说明已移除，统一以 `.env` 与当前运行配置为准。
- 当前真实入口仍为：`/admin/`、`/health`、`/api/admin/*`。
...
CODEX_PORT=8080
ADMIN_SECRET=dev-secret-change-before-shared-use  # 开发环境也必须设置
